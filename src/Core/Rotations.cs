...
namespace NCubeSolvers.Core
{
    public static class Rotations
    {
        ...
        {
            return AllRotations.First(r => r.Face == face && r.Direction == direction && r.LayerNumberFromFace == layerNumber);
        }

        public static FaceRotation ByFaceTwice(FaceType face, int layerNumber = 0)
        {
            return AllRotations.First(r => r.Face == face && r.Count == 2 && r.LayerNumberFromFace == layerNumber);
        }
    }
}
